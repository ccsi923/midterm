...
@SpringBootTest
class StudentCheckingControllerImplUnitTest {
    ...
    void setUp() {
        ...
        List<StudentChecking> studentCheckings = Collections.singletonList(studentChecking);
        ...
        when(studentCheckingRepository.findAll()).thenReturn(studentCheckings);
    }

    @Test
    @WithMockUser(roles = "ADMIN")
    void findAll() {
        List<StudentCheckingVM> studentCheckings = studentCheckingService.findAll();
        assertEquals("Pepe", studentCheckings.get(0).getPrimaryOwner().getName());
    }
