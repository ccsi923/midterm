...
class CreditCardControllerImplTest {
    ...
    void create_givenIdByParam() throws Exception {
        ...
        mockMvc.perform(post("/creditcard/?primary="+accountHolders.get(0).getId()+"&secondary="+accountHolders.get(1).getId())
                .with(user("admin").roles("ADMIN"))
                .content(objectMapper.writeValueAsString(accountRequest))
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated());
    ...
    void create_BadRequest() throws Exception {
        ...
        mockMvc.perform(post("/creditcard/?primary=1000&secondary=10000")
                .with(user("admin").roles("ADMIN"))
                .content(objectMapper.writeValueAsString(accountRequest))
                .contentType(MediaType.APPLICATION_JSON))
    ...
    void findAll() throws Exception {
        MvcResult result = mockMvc.perform(get("/creditcards")
